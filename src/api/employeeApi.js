// // src/api/employeeApi.js
// import axios from 'axios';
// import { BASE_URL } from './constants';
// // ✅ Create configured axios instance
// const axiosInstance = axios.create({
//   baseURL: BASE_URL,
//   maxRedirects: 1,
//   validateStatus: (status) => (status >= 200 && status < 303) || status === 302,
//   headers: {
//     'Content-Type': 'application/json',
//   },
// });
// // ✅ API functions
// // Create a new employee
// export const createEmployee = (userData, token) =>
//   axiosInstance.post('/employees/createemployee', userData, {
//     headers: { Authorization: `Bearer ${token}` },
//   });
// // Get employee by ID
// export const getEmployeeById = (id, token) =>
//   axiosInstance.get(`/employees/employees/${id}`, {
//     headers: { Authorization: `Bearer ${token}` },
//   });
// // Update employee
// // src/api/employeeApi.js
// export const updateEmployee = (id, userData, token) =>
//   axiosInstance.put(`/employees/updateemployee?id=${id}`, userData, {
//     headers: { Authorization: `Bearer ${token}` },
//   });
//   // errror 

// // Delete employee
// export const deleteEmployee = (id, token) =>
//   axiosInstance.delete(`/employees/${id}`, {
//     headers: { Authorization: `Bearer ${token}` },
//   });
// // Get all employees
// export const getAllEmployees = (token) =>
//   axiosInstance.get('/employees/getallemployees', {
//     headers: { Authorization: `Bearer ${token}` },
//   });




// src/api/employeeApi.js
import api from './axiosConfig';

export const getAllEmployees = () =>
  api.get('/employees/getallemployees');

export const getEmployeeById = (id) =>
  api.get(`/employees/employees/${id}`);

export const createEmployee = (userData) =>
  api.post('/employees/createemployee', userData);

export const updateEmployee = (id, userData) =>
  api.put(`/employees/updateemployee?id=${id}`, userData);

export const deleteEmployee = (id) =>
  api.delete(`/employees/deleteemployee?id=${id}`);