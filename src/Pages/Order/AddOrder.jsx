...
const initialClient = { id: "", name: "", email: "", phone: "", address: "", GSTCertificate: "", PAN: "" };
const initialOrder = {
  status: "CREATED",
  priority: "HIGH",
  createdAtDateTime: "",
...
export default function AddPostSale({ onClose }) {
  ...
  function handleRemoveOrder(idx) {
    setPostSales((ps) => {
      ...
      arr.splice(idx, 1);
      return { ...ps, orders: arr.length ? arr : [{ ...initialOrder, clientType: ps.clientType }] };
    });
    setOrderImages((img) => {
      const cpy = { ...img };
      delete cpy[idx];
  ...
  function handleImageChange(idx, files) {
    ...
    updateOrder(idx, "images", Array.from(files).map(f => f.name));
  }
  ...
  async function handleSubmit(e) {
    ...
    const ALLOWED_ORDER_FIELDS = [
      ...
      "orderStartDateTime", "status", "createdAtDateTime", "images",
      "id", "totalAmount", "media", "height", "gst", "qty",
      "totalAmountWithGST", "orderEndDateTime", "steps"
    ];
...
alert("Order Sucessfully added !")
  ...
  window.location.reload();
      ...
      setResult({ success: false, message: err?.response?.data?.message || "Failed to create post-sale" });
    }
    setSubmitting(false);
  }
              ...
              <div className={styles.optionsWrap}>
                {CLIENT_TYPES.map((ct) => (
                  <button
                    key={ct}
                    type="button"
                    className={`${styles.optionChip} ${postSales.clientType === ct ? styles.selected : ""}`}
                ...
                <div className={styles.row}>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) => setClient((c) => ({ ...c, GSTCertificate: e.target.value }))}
                  ...
                  <label>
                    ...
                    <input
                      type="text"
                      value={client.PAN}
                      onChange={(e) => setClient((c) => ({ ...c, PAN: e.target.value }))}
                      placeholder="PAN"
                    />
                  </label>
                </div>
            ...
            <div className={styles.row}>
              <label>
                ...
                <input
                  ...
                  onChange={(e) => updateField("postSalesdateTime", e.target.value)}
                />
              </label>
              <label>
                <span>Status</span>
...
function OrderConfigurator({
  ...
  const stepOptionLists = order.printType && printTypesObj[order.printType]
    ? Object.entries(printTypesObj[order.printType])
        ...
        .reduce((acc, [group, opts]) => { acc[group] = opts; return acc; }, {})
    : {};
  ...
  return (
    <div className={styles.orderCard}>
      ...
      <div className={styles.row}>
        <label>
          <span>Priority</span>
          <select value={order.priority} onChange={(e) => updateOrderField("priority", e.target.value)}>
        ...
        <label>
          <span>Status</span>
          <select value={order.status} onChange={(e) => updateOrderField("status", e.target.value)}>
            <option value="CREATED">CREATED</option>
  ...
  <div>
    ...
    <input
      type="number"
      min={0}
      value={order.unitPrice !== undefined && order.unitPrice !== null && order.unitPrice !== 0
        ? order.unitPrice
        : unitPrice}
      onChange={e => {
  ...
  <div>
    ...
    <input
      type="number"
      min={0}
      value={order.totalAmount !== undefined && order.totalAmount !== null && order.totalAmount !== 0
        ? order.totalAmount
        : totalAmount}
      onChange={e => {
        const value = Number(e.target.value);
  ...
  <div>
    ...
    <input
      type="number"
      min={0}
      value={order.totalAmountWithGST !== undefined && order.totalAmountWithGST !== null && order.totalAmountWithGST !== 0
